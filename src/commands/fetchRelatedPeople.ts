...
export const fetchRelatedPeople = async (name: string): Promise<Resource[]> => {
    ...
    const relevantPeople = await Promise.all([
        search<SearchResults<Film>>("films", name),
        search<SearchResults<Planet>>("planets", name),
        search<SearchResults<SpeciesImpl>>("species", name),
        search<SearchResults<Vehicle>>("vehicles", name),
        search<SearchResults<Starship>>("starships", name)
